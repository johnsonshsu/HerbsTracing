...
{
    public class BASP008Controller : Controller
    {
        /// <summary>
        /// 資料初始事件
        /// </summary>
        /// <returns></returns>
        /// [HttpGet]
        public IActionResult Init(string id = "W")
        {
            //這裏可以寫入初始程式
            SessionService.PrgNo = "BASP008"; //預設程式編號
            SessionService.PrgName = "檢測項目主檔維護"; //預設程式名稱
            SessionService.BaseNo = id; //預設程式代碼
            SessionService.PageMasterSize = 10; //預設表頭每頁筆數
            SessionService.SearchText = "";
            SessionService.SortColumn = "z_bas_test_item.mno";
        ...
        public IActionResult Index(int id = 1)
        {
            ...
            //取得資料列表集合
            using var sqlData = new sql_z_bas_test_item();
            var model = sqlData.GetDataListByCode(SessionService.BaseNo);
        ...
        /// <returns></returns>
        [HttpPost]
        public IActionResult CreateEdit(z_bas_test_item model)
        {
