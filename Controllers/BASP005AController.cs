...
{
    public class BASP005AController : Controller
    {
        /// <summary>
        /// 資料初始事件
        /// </summary>
        ...
        public IActionResult Init(string id = "")
        {
            ...
            SessionService.PrgNo = "BASP005A"; //預設程式編號
            SessionService.PrgName = "農戶契作合約維護"; //預設程式名稱
        ...
        public IActionResult Index(int id = 1)
        {
            ...
            var vendorModel = vendor.GetDataByNo(SessionService.BaseNo);
            string vendorInfo = (vendorModel == null) ? "" : $"{vendorModel.mno} {vendorModel.mname}";
            ActionService.SetActionName(enAction.Index);
            ActionService.SetSubActionName(vendorInfo);
            ActionService.SetActionCardSize(enCardSize.Max);
            //取得資料列表集合
            using var sqlData = new sql_z_bas_contract();
            //農戶(F)且已核准(B)
            var model = sqlData.GetUserContractList(SessionService.BaseNo, SessionService.SearchText).ToPagedList(id, pageSize);
        ...
        public IActionResult CreateEdit(z_bas_contract model)
        {
            ...
            if (rowId== 0)
            {
                model.user_no = SessionService.BaseNo; //使用者編號
