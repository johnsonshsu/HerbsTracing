...
{
    public class BASP006AController : Controller
    {
        /// <summary>
        /// 資料初始事件
        /// </summary>
        ...
        public IActionResult Init(string id = "")
        {
            ...
            SessionService.PrgNo = "BASP006A"; //預設程式編號
            SessionService.PrgName = "料品組成資料維護"; //預設程式名稱
            SessionService.PageMasterSize = 10; //預設表頭每頁筆數
            SessionService.SearchText = "";
            SessionService.SortColumn = "z_bas_item_bom.mno";
        ...
        public IActionResult Index(int id = 1)
        {
            ...
            using var item = new sql_z_bas_item();
            var itemModel = item.GetDataByNo(SessionService.BaseNo);
            string itemInfo = (itemModel == null) ? "" : $"{itemModel.mno} {itemModel.mname}";
            ActionService.SetActionName(enAction.Index);
            ActionService.SetSubActionName(itemInfo);
        ...
        /// <returns></returns>
        [HttpPost]
        public IActionResult CreateEdit(z_bas_item_bom model)
        {
            //檢查是否有違反 Metadata 中的 Validation 驗證
            if (!ModelState.IsValid) return View(model);
            //執行新增或修改資料
            using var sqlItem = new sql_z_bas_item();
            using var sqlData = new sql_z_bas_item_bom();
            int rowId = string.IsNullOrEmpty(model.rowid.ToString()) ? 0 : 1;
            if (rowId == 0)
            {
                var itemModel = sqlItem.GetDataByNo(SessionService.BaseNo ?? "");
                model.parentid = itemModel.rowid; //品項主鍵
                model.item_no = SessionService.BaseNo; //使用者編號
