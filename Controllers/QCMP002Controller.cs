...
{
    public class QCMP002Controller : Controller
    {
        /// <summary>
        /// 資料初始事件
        /// </summary>
        ...
        {
            //這裏可以寫入初始程式
            SessionService.BaseNo = "W";
            SessionService.PrgNo = "QCMP002"; //預設程式編號
            SessionService.PrgName = "產地水質檢驗記錄"; //預設程式名稱
            SessionService.PageMasterSize = 10; //預設表頭每頁筆數
            SessionService.SearchText = "";
            SessionService.SortColumn = "z_qcm_environment.mno";
        ...
        public IActionResult Index(int id = 1)
        {
            ...
            //取得資料列表集合
            using var sqlData = new sql_z_qcm_environment();
            //農戶(F)且已核准(B)
            var model = sqlData.GetDataList(SessionService.BaseNo , SessionService.SearchText).ToPagedList(id, pageSize);
        ...
        public IActionResult CreateEdit(string id = "")
        {
            ...
            var model = new z_qcm_environment();
            if (string.IsNullOrEmpty(id) || id == "0")
            {
                ...
                model.mno = "";
                model.mdate = DateTime.Today;
                model.sdate = DateTime.Today;
                model.handle_no = "";
                model.remark = "True";
            }
            else
        ...
        /// <returns></returns>
        [HttpPost]
        public IActionResult CreateEdit(z_qcm_environment model)
        {
        ...
        {
            return RedirectToAction(ActionService.Init, "QCMP002A", new { area = ActionService.Area, id = id });
        }

        [HttpGet]
        public IActionResult Detail(string codeNo , string mno)
        {
            SessionService.ParentPage = SessionService.PageMaster;
            SessionService.BaseNo = codeNo;
            SessionService.CategoryNo = mno;
            return RedirectToAction(ActionService.Init, "QCMP002A", new { area = ActionService.Area});
        }
    }
}
