...
{
    public class PRGP002Controller : Controller
    {
        public sql_z_sys_program SqlDatas { get; set; } = new sql_z_sys_program();
        public z_sys_program Model { get; set; } = new z_sys_program();
        ...
        {
            //這裏可以寫入初始程式
            SessionService.PrgNo = "PRGP002"; //預設程式編號
            SessionService.PrgName = "系統程式資料維護"; //預設程式名稱
            SessionService.PageMasterSize = 10; //預設表頭每頁筆數
            SessionService.SearchText = "";
            SessionService.SortColumn = "z_sys_program.moduleno, z_sys_program.msort, z_sys_program.mno";
            SessionService.SortDirection = "ASC,ASC,ASC";
            //返回資料列表
        ...
        public IActionResult CreateEdit(string id = "")
        {
            ...
            {
                //取得新增或修改的員工資料結構及資料
                using var sqlData = new sql_z_sys_program();
        ...
        /// <returns></returns>
        [HttpPost]
        public IActionResult CreateEdit(z_sys_program model)
        {
