...
{
    public class QCMP009Controller : Controller
    {
        /// <summary>
        /// 資料初始事件
        /// </summary>
        ...
        [HttpGet]
        [Route("{controller}/{action}/{id}/{mno}")]
        public IActionResult Detail(string id , string mno)
        {
            SessionService.ParentPage = SessionService.PageMaster;
            SessionService.BaseNo = mno;
            SessionService.CategoryNo = id;
            return RedirectToAction(ActionService.Init, "QCMP009A", new { area = ActionService.Area});
        }
    }
}
