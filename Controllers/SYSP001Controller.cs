...
{
    public class SYSP001Controller : Controller
    {
        public sql_z_sys_application SqlDatas { get; set; } = new sql_z_sys_application();
        public z_sys_application Model { get; set; } = new z_sys_application();

        public SYSP001Controller()
        {
            using var sqlData = new sql_z_sys_application();
            sqlData.CheckApplicationData();
        }
        ...
        {
            //這裏可以寫入初始程式
            SessionService.PrgNo = "SYSP001"; //預設程式編號
            SessionService.PrgName = "系統名稱資料維護"; //預設程式名稱
            SessionService.PageMasterSize = 10; //預設表頭每頁筆數
            SessionService.SearchText = "";
            SessionService.SortColumn = "z_sys_application.app_name,z_sys_application.app_version";
            SessionService.SortDirection = "ASC,ASC";
        ...
        public IActionResult Index(int id = 1)
        {
            ...
            //using var sqlData = new sql_z_sys_application();
            //農戶(F)且已核准(B)
            var model = SqlDatas.GetDataList(SessionService.SearchText).ToPagedList(id, pageSize);
        ...
        /// <returns></returns>
        [HttpPost]
        public IActionResult CreateEdit(z_sys_application model)
        {
