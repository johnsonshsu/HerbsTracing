...
{
    public class QCMP001AController : Controller
    {
        /// <summary>
        /// 資料初始事件
        /// </summary>
        ...
        {
            //這裏可以寫入初始程式
            SessionService.PrgNo = "QCMP001"; //預設程式編號
            SessionService.PrgName = "藥材種植工作紀錄"; //預設程式名稱
            SessionService.PageMasterSize = 10; //預設表頭每頁筆數
            SessionService.SearchText = "";
            SessionService.SortColumn = "z_bas_record.lot_no";
            SessionService.SortDirection = "DESC";
        ...
        public IActionResult Index(int id = 1)
        {
            ...
            string parentInfo = $"產地代號：{SessionService.StringValue1}，藥材代號：{  SessionService.StringValue2}，批號：{ SessionService.StringValue3}";
            SessionService.PageMaster = id;
            ActionService.SetActionName(enAction.Index);
            ActionService.SetSubActionName(parentInfo);
            ActionService.SetActionCardSize(enCardSize.Max);
            //取得資料列表集合
            using var sqlData = new sql_z_bas_record();
            //農戶(F)且已核准(B)
            var model = sqlData.GetPlaceItemLotDetailList(SessionService.SearchText).ToPagedList(id, pageSize);
        ...
        /// <returns></returns>
        [HttpPost]
        public IActionResult CreateEdit(z_bas_record model)
        {
            ...
            model.vender_no = SessionService.BaseNo;
            model.place_no = SessionService.StringValue1;
            model.item_no = SessionService.StringValue2;
            model.lot_no = SessionService.StringValue3;
        ...
        {
            return RedirectToAction(ActionService.Init, "QCMP001A", new { area = ActionService.Area, id = id });
        }
    }
}
