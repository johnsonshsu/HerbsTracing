...
{
    public class PRGP001Controller : Controller
    {
        public sql_z_sys_module SqlDatas { get; set; } = new sql_z_sys_module();
        public z_sys_module Model { get; set; } = new z_sys_module();
        ...
        {
            //這裏可以寫入初始程式
            SessionService.PrgNo = "PRGP001"; //預設程式編號
            SessionService.PrgName = "系統模組資料維護"; //預設程式名稱
            SessionService.PageMasterSize = 10; //預設表頭每頁筆數
            SessionService.SearchText = "";
            SessionService.SortColumn = "z_sys_module.mcode, z_sys_module.msort,z_sys_module.mno";
        ...
        public IActionResult Index(int id = 1)
        {
            ...
            //取得資料列表集合
            //using var sqlData = new sql_z_sys_module();
            //農戶(F)且已核准(B)
        ...
        /// <returns></returns>
        [HttpPost]
        public IActionResult CreateEdit(z_sys_module model)
        {
