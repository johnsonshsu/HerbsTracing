...
{
    public class QCMP006Controller : Controller
    {
        /// <summary>
        /// 資料初始事件
        /// </summary>
        ...
        {
            //這裏可以寫入初始程式
            SessionService.PrgNo = "QCMP006"; //預設程式編號
            SessionService.PrgName = "成品檢驗記錄維護"; //預設程式名稱
            SessionService.PageMasterSize = 10; //預設表頭每頁筆數
            SessionService.SearchText = "";
            SessionService.SortColumn = "z_qcm_finish.mno";
        ...
        public IActionResult Index(int id = 1)
        {
            ...
            //取得資料列表集合
            using var sqlData = new sql_z_qcm_finish();
            //農戶(F)且已核准(B)
        ...
        /// <returns></returns>
        [HttpPost]
        public IActionResult CreateEdit(z_qcm_finish model)
        {
        ...
        public IActionResult Detail(string id)
        {
            ...
            return RedirectToAction(ActionService.Init, "QCMP006A", new { area = ActionService.Area});
        }

        [HttpGet]
        [Route("{controller}/{action}/{mno}/{categoryNo}")]
        public IActionResult Bom(string mno , string categoryNo)
        {
            SessionService.ParentPage = SessionService.PageMaster;
            SessionService.BaseNo = mno;
            SessionService.CategoryNo = categoryNo;
            return RedirectToAction(ActionService.Init, "QCMP006B", new { area = ActionService.Area});
        }
    }
}
